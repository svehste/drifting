...
export default async function ScoringPage({ params }: { params: { id: string } }) {
    ...
    if (err instanceof AuthzError) {
      return (
        <main className="container">
          <h1>{nb.raceStatus.qualifying}</h1>
          <p className="error">{err.message}</p>
          <p>
            <Link href="/logg-inn">{nb.nav.signIn}</Link>
          </p>
        </main>
      );
    }
  ...
  const data = await getScoringData(params.id);
  if (!data) notFound();
  ...
  if (user.roles.includes("admin")) {
    ...
    const duties = await db
      ...
      .where(
        and(
          eq(raceOfficials.raceId, params.id),
  ...
  return (
    <main className="container">
      <div className="leaderboard-head">
        <h1>{data.raceName}</h1>
        <Link href={`/lop/${data.raceId}/resultater`} className="btn-secondary">
          {nb.leaderboard.title}
        </Link>
      </div>
      <p className="muted">
        {nb.raceStatus.qualifying} · Du scorer:{" "}
